...
const seedDatabase = async () => {
  try {
    ...
    for (let c = 0; c < classes.length; c++) {
      ...
      const startTime1 = `${9 + c}:00 AM`;
      const endTime1 = `${10 + c}:00 AM`;
      const startTime2 = `${10 + c}:00 AM`;
      const endTime2 = `${11 + c}:00 AM`;
      
      await Timetable.create({ classId: cls._id, section: 'A', subjectId: clsSubjects[0]._id, teacherId: teachers[0]._id, day: 'Monday', startTime: startTime1, endTime: endTime1, roomNumber: '101', schoolId: school._id });
      await Timetable.create({ classId: cls._id, section: 'A', subjectId: clsSubjects[1]._id, teacherId: teachers[1]._id, day: 'Monday', startTime: startTime2, endTime: endTime2, roomNumber: '102', schoolId: school._id });
