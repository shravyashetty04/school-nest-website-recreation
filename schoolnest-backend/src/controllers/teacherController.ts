...
export const bulkCreateTeachers = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.file) {
      res.status(400);
      ...
      .on('end', async () => {
        try {
          ...
          for (const row of results) {
            ...
            if (!firstName || !lastName || !email || !phone) continue; // Skip invalid rows
            ...
            const teacher = await Teacher.create({
              ...
              teacherId, employeeId, schoolId, userId: user._id
            });
            insertedTeachers.push(teacher);
          }
          res.status(201).json({ success: true, count: insertedTeachers.length });
        } catch (error) {
          next(error);
        }
      });
